...
import {Wallet, EventTargetImpl, helper, spectrecore, CONFIRMATION_COUNT, COINBASE_CFM_COUNT} from '@spectre/wallet';
const {HDPrivateKey} = spectrecore;
...
export const initSpectreFramework = (opt:{workerPath?:string}={})=>{
	return new Promise<void>((resolve, reject)=>{
		helper.dpc(2000, ()=>{
			...
			if(IS_NODE_CLI){
				...
				let {
					workerPath="/node_modules/@spectre/wallet-worker/worker.js"
				} = opt
				url = new URL(workerPath, baseURL);
			}
			workerLog.info("initSpectreFramework", url, baseURL)
...
class WalletWrapper extends EventTargetImpl{
	...
	initWorker(){
		if(!worker)
			throw new Error("Please init spectre framework using 'await initSpectreFramework();'.")
		this.worker = worker;
